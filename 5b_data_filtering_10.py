...
def main(spark, userID):
    ...
    songs_filtered10 = grouped_by_items.filter(grouped_by_items.user_count_per_song > 10)
    # print('SONGS CREATED')
    
    songs_filtered10.createOrReplaceTempView('songs_filtered10')
                                           
    query_songs = "SELECT COUNT(*) FROM songs_filtered10"
    ...
    utility_train_filtered = df_with_count_train.filter(df_with_count_train.user_count > 10).drop("user_count")
    utility_val_filtered = df_with_count_val.filter(df_with_count_val.user_count > 10).drop("user_count")
    ...
    # utility_train_filtered.write.mode('overwrite').parquet('utility_train_filtered10.parquet')
    # print('WRITTEN TRAIN')
    # utility_val_filtered.write.mode('overwrite').parquet('utility_val_filtered10.parquet')
    # print('WRITTEN VAL')
    ...
    utility_train_filtered_rep.write.mode('overwrite').parquet('utility_train_filtered10_rep.parquet')
    print('WRITTEN TRAIN')
    utility_val_filtered_rep.write.mode('overwrite').parquet('utility_val_filtered10_rep.parquet')
    print('WRITTEN VAL')
