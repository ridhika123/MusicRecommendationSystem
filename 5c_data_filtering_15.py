...
def main(spark, userID):
    ...
    songs_filtered15 = grouped_by_items.filter(grouped_by_items.user_count_per_song > 15)
    # print('SONGS CREATED')
    
    songs_filtered15.createOrReplaceTempView('songs_filtered15')
                                           
    query_songs = "SELECT COUNT(*) FROM songs_filtered15"
    ...
    utility_train_filtered = df_with_count_train.filter(df_with_count_train.user_count > 15).drop("user_count")
    utility_val_filtered = df_with_count_val.filter(df_with_count_val.user_count > 15).drop("user_count")
    ...
    utility_train_filtered.write.mode('overwrite').parquet('utility_train_filtered15.parquet')
    print('WRITTEN TRAIN')
    utility_val_filtered.write.mode('overwrite').parquet('utility_val_filtered15.parquet')
    print('WRITTEN VAL')
