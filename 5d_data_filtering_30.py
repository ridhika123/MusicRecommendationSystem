...
def main(spark, userID):
    ...
    songs_filtered30 = grouped_by_items.filter(grouped_by_items.user_count_per_song > 30)
    # print('SONGS CREATED')
    
    songs_filtered30.createOrReplaceTempView('songs_filtered30')
                                           
    query_songs = "SELECT COUNT(*) FROM songs_filtered30"
    ...
    utility_train_filtered = df_with_count_train.filter(df_with_count_train.user_count > 30).drop("user_count")
    utility_val_filtered = df_with_count_val.filter(df_with_count_val.user_count > 30).drop("user_count")
    ...
    utility_train_filtered.write.mode('overwrite').parquet('utility_train_filtered30.parquet')
    print('WRITTEN TRAIN')
    utility_val_filtered.write.mode('overwrite').parquet('utility_val_filtered30.parquet')
    print('WRITTEN VAL')
